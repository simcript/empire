...
export function deriveExecutablePath(displayIcon, installLocation) {
  ...
  if (iconPath && iconPath.endsWith(".exe") && existsSync(iconPath))
    return iconPath;
...
export async function listInstalledPrograms() {
  ...
  const registryData = await Promise.all(
    REGISTRY_PATHS.map((path) => readRegistryTree(path))
  );
  ...
  for (const entry of flattened) {
    ...
    if (FILTER_SYSTEM_PROGRAMS && (lname.includes("windows") || lname.includes("security update") || lname.includes("update for") || lname.includes("hotfix"))) continue;
    if (FILTER_MICROSOFT_RUNTIMES && (lname.includes("microsoft visual c++") || lname.includes(".net framework") || lname.includes("microsoft redistributable"))) continue;
    if (FILTER_WINDOWS_UPDATES && (lname.includes("windows update") || lname.includes("update"))) continue;
    if (FILTER_DRIVERS && (lname.includes("driver") || lname.includes("graphics driver") || lname.includes("audio driver"))) continue;
    if (FILTER_SERVICES && (lname.includes("service pack") || lname.includes("background") || lname.includes("server"))) continue;
    ...
    programs.push({
      ...
      estimatedSize: entry.EstimatedSize
        ? parseInt(entry.EstimatedSize, 10)
        : undefined
    });
  }
